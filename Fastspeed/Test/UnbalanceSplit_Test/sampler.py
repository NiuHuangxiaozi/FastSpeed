...
class Distributed_Elastic_Sampler(Sampler[T_co]):
    def __init__(self,
        ...
        self.partition_strategy=partition_strategy #这里是我设置的一些策略，uneven就是代表不均匀的划分
        ...
        self.seed = seed
    def auto_partition(self): #不均匀划分数据集的实现
        if self.partition_strategy["method"]=="uneven":
            #self.num_samples表示的是sample的list,例如：cifar10有50000个训练样例，self.num_samples=[40000,10000]代表的就是GPU0训练40000个样例，GPU1训练10000个样例
            self.num_samples= self.partition_strategy["partition_list"]
            ...
            assert(self.partition_strategy["method"]=="uneven")
