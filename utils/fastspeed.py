...
from utils.calculation import calculate_time
from utils.resource_detection import Test_MaxBatchsize
from utils.load_json import Params
from argparse import Namespace
from utils.scheduling import  batchsize_scheduling
# exception
            ...
            if m_type == 'ddp':
                ...
                if self.config.strategy["unbalanced_strategy"] == "time":
                    ...
                    dummy_time = calculate_time(dummy_model,
                                                ...
                                                self.config.data["data_dummy_shape"]
                                                )
                    dummy_time_tensor = torch.tensor(dummy_time).to(self.dist_args.local_rank)

                    time_list = [torch.zeros(1, ).to(self.dist_args.local_rank) for i in range(0, self.dist_args.world_size)]
    ...
    def unbalanced_datasplit(self ,dataset ,model) -> torch.utils.data.DataLoader:
        ...
        try:
            if partition_type== 'manual' or partition_type == 'autobalanced':
                ...
                raise MyException(1, "Your choice of partition_method(" +partition_type + ") is wrong [neither manual or autobalanced!")
                ########################################################################################################
        except MyException as e:
            print(e)
            sys.exit(1)
